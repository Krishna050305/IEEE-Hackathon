...
# MongoDB Setup
conn = MongoClient("mongodb://localhost:27017/AppointmentBooking")
...
async def patient_dashboard(request: Request):
    ...
    user = patient_collection.find_one({"_id": ObjectId(user_id)})
    return templates.TemplateResponse("patient/dashboard.html", {"request": request, "user": user})
...
async def post_doctor_login(
    ...
    request.session["user_id"] = str(doctor["_id"])
